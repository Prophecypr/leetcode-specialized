...
| 求最长 | 求最短 | 求个数 |
|--------|--------|--------|
| `max(ans, len)` | `min(ans, len)` | **`ans += len`** |

**核心公式：** `ans += right - left + 1`

以 right 结尾、乘积 < k 的子数组有多少个？
→ 当窗口 `[left, right]` 乘积全部 < k 时，以 right 结尾的所有 `right - left + 1` 个子数组都满足条件。

**图解：**
```
nums = [10, 5, 2, 6], k = 100
right=0: [10]           → +1  (10<100)
right=1: [5],[10,5]     → +2  (5<100, 50<100)
right=2: prod=100≥100  → 缩到 [2] → +1
right=3: [2],[2,6]      → +2  (12<100)
ans = 1+2+1+2 = 6 ✓
```

**注意：** `k <= 1` 时直接返回 0（正整数乘积不可能 < 1）
...
| 不定长·最短 | 扩到合法→缩到不合法前更新 | 209. 最短子数组和 |
| **求子数组个数** | **`ans += right-left+1`** | **713. 乘积 < K** |
| 前缀和+窗口 | 用前缀和数组快速计算区间和 | — |
